...
export default async function BookingConfirmed({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  if (!start || !name || !email || !minutes) {
    return (
      <Section tone="page" py="var(--space-9)">
        ...
        <Rule />
        <p style={{ marginTop: "var(--space-5)", maxWidth: "46ch" }}>We couldn&rsquo;t find your booking details. If you were just charged, contact info@gentleinspirer.com and we&rsquo;ll sort it out.</p>
  ...
  return (
    <Section tone="page" py="var(--space-9)">
      ...
      {!result.calBooked && <p style={{ marginTop: "var(--space-3)", fontSize: "var(--size-caption)", color: "var(--text-muted)" }}>Calendar hold pending — if you don&rsquo;t hear from us within a day, email info@gentleinspirer.com.</p>}
      <div style={{ marginTop: "var(--space-6)" }}>
        <Button variant="secondary" href="/book">
          Book another
        </Button>
